...
- **记忆图谱**：以**人为节点、事件为连接**的知识图谱——回答"什么人和谁在什么地方发生了什么事情"。用户与身边的人是节点，情景记忆是把人连起来的**事件**（卡片），事件再连到它发生的**地点**；两个人通过共同事件相连（"和大壮在操场看星星"→ 用户·大壮 同挂那件事上）。喜好/雷区/最在乎/专属梗作为语义档案轻量挂在用户旁。力导向布局，可拖动、缩放、点开看细节。
- **关系档案**：养成门槛、信任账本、PAD 心境、人格磨合、承诺账本、每轮记忆调用痕迹，以及"指令试驾"（输入一句话，干跑看引擎此刻怎么派戏）。

为支撑这套可视化，引擎补了两处"关系/事件的真实历史"：
- `UserRelationState.timeline`——每个关系事件都钉上当下的信任/亲密/阶段快照（轨迹曲线的来源）；
- 每条情景记忆落库时记下它**涉及的人**（`Episode.actors`，泛称按档案唯一解析到人名）和**发生的地点**（`Episode.place`，闭集词典高精度抽取）——事件图谱的人·地之边就长在这里。

安全红线进 UI：危机相关记忆只在时间线留一个**不含内容**的封存标记，且**永不进入图谱**，封存区内容不出现在任何接口里。
