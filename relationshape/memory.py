...
def _strip_particles(item: str) -> str:
    """去掉抽取结果头尾的连接词/语气词/指代尾巴：
    "的就是足球"→"足球"；"但蘑菇"→"蘑菇"；"堆雪人这件事"→"堆雪人"（尾部指代短语，
    否则与"不爱堆雪人"对不上、跨轮撤回失效）；尾部"就"（"石头就"→"石头"）。"""
    item = re.sub(r"^(?:的就是|的是|就是|的|是)", "", item or "")
    item = _LEAD_CONJ_RE.sub("", item)
    item = re.sub(r"(?:这件事儿?|这事儿?|这活儿?|这玩意儿?|这东西|就成|就好|就行|就可以)+$", "", item)
...
def _extract_user_name(text: str) -> str | None:
    for pattern, validator in _NAME_RES:
        ...
        if not m:
            continue
        name = re.sub(r"(就成|就好|就行|就可以|就得了|吗|呢|呀|哦|吧|啦|嘛)+$", "", m.group(1).strip())
...
_ADV = r"(?:一点(?:都|也)?|一口(?:都|也)?|半点(?:都|也)?|压根|根本|从来|向来|真的?|实在|确实|其实|完全|特别|可|就|也)?"
...
_NAME_RES = [
    ...
    # 裸"我是X"有歧义（我是学生/好人；"葱花我是真心讨厌"会把"真心讨厌"当名字）→ 保守排除
    # 常见谓词/程度副词/情绪动词起始（这些后面跟的是状态/感受，不是名字）
    (
        re.compile(r"我是(?!谁|什么|啥|哪|何|一个|一名|个|在|想|很|不|没|来|说|觉得"
                   r"|真|特|超|最|可|挺|蛮|太|更|讨厌|喜欢|爱|怕|烦|受不了)([^\s，。！？!?]{1,20})(?:[，,。！？!?]|岁|$)"),
...
# 关系泛称集合：必须与下方 _RELG 正则同步——关系词永不当人名（日志：'死党'/'队友'被记成人名）
_REL_WORDS = {"朋友", "同桌", "同学", "老师", "哥哥", "姐姐", "弟弟", "妹妹",
              "闺蜜", "发小", "邻居", "队友", "死党"}
...
_QATTR = r"([^，。！？!?的\s]{0,8})"
_QNAME = r"([^，。！？!?的了是就\s叫]{1,5})"   # 排除"就"，否则"石头就是我的好队友"吞成"石头就"
